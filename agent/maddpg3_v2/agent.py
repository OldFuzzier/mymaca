...
if IS_DISPERSED:
    from agent.maddpg3_v2.maddpg_dispersed import MADDPG
else:
    from agent.maddpg3_v2.maddpg import MADDPG

DETECTOR_NUM = 0
FIGHTER_NUM = 10
OBS_NUM = 2 + 1 + 2 * 10 + 10 + 10 + 20
...
class Agent(BaseAgent):
    def __init__(self):
        ...
        self.obs_ind = 'maddpg3_v2'
        if not os.path.exists('model/{}/{}0.pkl'.format(MODEL_PATH, MODEL_NAME)):
            logger.info('Error: agent maddpg3_v2 model data not exist!')
            exit(1)
    ...
    def get_action(self, obs_dict, step_cnt):
        ...
        for y in range(self.fighter_num):
            ...
            if step_cnt > STEP_BEFORE_TRAIN:
                ...
                obs = np.concatenate((course, pos, r_visible_pos, j_visible_dir, striking_id, g_visible_pos), axis=0)
            ...
            if obs_dict['fighter'][y]['alive']:
                ...
                # if step_cnt > STEP_BEFORE_TRAIN and not any(striking_id):
