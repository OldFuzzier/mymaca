...
from config import logger, MODEL_NAME, IS_DISPERSED
from utils import action2direction
from rule.agentutil4 import fighter_rule

if IS_DISPERSED:
    from agent.maddpg3.maddpg_dispersed import MADDPG
else:
    from agent.maddpg3.maddpg import MADDPG  # class
...
class Agent(BaseAgent):
    def __init__(self):
        ...
        self.obs_ind = 'maddpg3'
        if not os.path.exists('model/maddpg3/{}0.pkl'.format(MODEL_NAME)):
            logger.info('Error: agent maddpg3 model data not exist!')
            exit(1)
    ...
    def get_action(self, obs_dict, step_cnt):
        ...
        for y in range(self.fighter_num):
            ...
            tmp_g_visible_pos = obs_dict['fighter'][y]['g_visible_pos']  # (10, 2)  # rule use
