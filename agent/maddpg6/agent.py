...
from utils import action2direction2
from rule.agentutil4 import fighter_rule

if IS_DISPERSED:
    from agent.maddpg6.maddpg_dispersed import MADDPG
else:
    from agent.maddpg6.maddpg import MADDPG

DETECTOR_NUM = 0
FIGHTER_NUM = 10
OBS_NUM = 2 + 1 + 2 * 10 + 10 + 2 * 10
ACTION_NUM = 7 if IS_DISPERSED else 1  # # dispersed num, ddpg
STEP_BEFORE_TRAIN = 0
...
class Agent(BaseAgent):
    ...
    def get_action(self, obs_dict, step_cnt):
        ...
        for y in range(self.fighter_num):
            ...
            tmp_g_visible_pos = obs_dict['fighter'][y]['g_striking_pos']  # (10, 2)
            ...
            if step_cnt > STEP_BEFORE_TRAIN:
                ...
                obs = np.concatenate((course, pos, r_visible_pos, j_visible_dir, g_visible_pos), axis=0)
            ...
            if obs_dict['fighter'][y]['alive']:
                ...
                #         [any(r_visible_pos >= 0), any(j_visible_dir >= 0), any(g_visible_pos >= 0)]):
                if any(j_visible_dir >= 0):
                    tmp_action = self.maddpg.select_action(y, obs)
                    logger.info('tmp action: {}'.format(tmp_action))
                    # 添加动作, 将动作转换为偏角
                    tmp_action_i = np.argmax(tmp_action)
                    logger.info('tmp action i: {}'.format(tmp_action_i))
                    true_action[0] = action2direction2(true_action[0], tmp_action_i)
                    logger.info('fighter{} course: {}'.format(y+1, true_action[0]))
                    # logger.wait()
