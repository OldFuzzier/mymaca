...
if IS_DISPERSED:
    from agent.maddpg4_stable.maddpg_dispersed import MADDPG
else:
    from agent.maddpg4_stable_stable.maddpg import MADDPG
...
class Agent(BaseAgent):
    def __init__(self):
        ...
        self.obs_ind = 'maddpg4_stable'
        if not os.path.exists('model/{}/{}0.pkl'.format(MODEL_PATH, MODEL_NAME)):
            logger.info('Error: agent maddpg4_stable model data not exist!')
            exit(1)
