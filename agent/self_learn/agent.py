...
if IS_DISPERSED:
    from agent.self_learn.maddpg_dispersed import MADDPG
else:
    from agent.self_learn.maddpg import MADDPG
...
ACTION_NUM = 360 if IS_DISPERSED else 1  # # dispersed num, ddpg
STEP_BEFORE_TRAIN = 0
...
class Agent(BaseAgent):
    def __init__(self):
        ...
        self.obs_ind = 'self_learn'
        if not os.path.exists('model/{}/{}0.pkl'.format(MODEL_PATH, MODEL_NAME)):
            logger.info('Error: agent self_learn model data not exist!')
            exit(1)
    ...
    def get_action(self, obs_dict, step_cnt):
        ...
        for y in range(self.fighter_num):
            ...
            if step_cnt > STEP_BEFORE_TRAIN:
                ...
                obs = np.concatenate((course, pos, l_missile, s_missile, r_visible_pos, j_visible_dir, striking_id),
                                     axis=0)
            ...
            if obs_dict['fighter'][y]['alive']:
                ...
                # model policy
                if step_cnt > STEP_BEFORE_TRAIN and not any([any(r_visible_pos >= 0), any(j_visible_dir >= 0)]):
