...
from train.maddpg6.MADDPG import MADDPG
from interface import Environment
# from agent.fix_rule.agent import Agent  # todo 更改规则模型
# from agent.MidSolo.agent import Agent
from agent.self_rule.agent_20200723 import Agent
from config import logger, IS_DISPERSED, PICS_PATH
from rule.agentutil4 import fighter_rule
from utils import reward_w, loss_w, action2direction2

MAX_EPOCH = 10000  # 2000
MAX_STEP = 800  # default 5000 after: 800DDA
STEP_BEFORE_TRAIN = 0  # 前多少步不需要
BATCH_SIZE = 256  # 256
...
AGENT_NAME = 'maddpg6'
# obs: 自身坐标，自身航向，主动观测列表坐标，被动观测航向，全局观测坐标
...
AGENT_NUM = 2
COURSE_NUM = 16
ACTION_NUM = 7 if IS_DISPERSED else 1  # dispersed num or ddpg
        ...
        while True:
            ...
            for y in range(red_fighter_num):
                ...
                if not red_obs_dict['fighter'][y]['alive']:
                    ...
                    true_action[1] = y + 1  # todo 添加频点
                    logger.debug('true action rule out: {}'.format(true_action))
                    # 添加动作, 将动作转换为偏角
                    if step_cnt > STEP_BEFORE_TRAIN:
                        # model policy
                        if not any(j_visible_dir >= 0):
