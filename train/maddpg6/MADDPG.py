...
from train.maddpg6.memory import ReplayMemory, Experience
from config import GPU_CONFIG, logger, MODEL_PATH
from train.maddpg6.model_dispersed import Critic, Actor
...
def hard_update(target, source, t=None):
    for target_param, source_param in zip(target.parameters(),
                                          source.parameters()):
...
class MADDPG:
        ...
        if self.use_cuda:
            ...
            for i in range(len(self.actors)):
                ...
                if GPU_CONFIG.use_parallel:
                    self.actors[i] = nn.parallel.DistributedDataParallel(self.actors[i], device_ids=GPU_CONFIG.device_ids)
            for i in range(len(self.critics)):
                # self.critics[i].to(GPU_CONFIG.device)
                self.critics[i] = self.critics[i].cuda()
                if GPU_CONFIG.use_parallel:
                    self.critics[i] = nn.parallel.DistributedDataParallel(self.critics[i], device_ids=GPU_CONFIG.device_ids)
            for i in range(len(self.actors_target)):
                ...
                if GPU_CONFIG.use_parallel:
                    self.actors_target[i] = nn.parallel.DistributedDataParallel(self.actors_target[i], device_ids=GPU_CONFIG.device_ids)
            for i in range(len(self.critics_target)):
                ...
                if GPU_CONFIG.use_parallel:
                    self.critics_target[i] = nn.parallel.DistributedDataParallel(self.critics_target[i], device_ids=GPU_CONFIG.device_ids)
    ...
    def update_policy(self):
        ...
        if self.learn_step_counter > 0 and self.learn_step_counter % self.replace_target_iter == 0:  # todo
            ...
            for i in range(self.n_agents):
                # replace
                soft_update(self.critics_target[i], self.critics[i], self.tau)  # todo soft or hard
                soft_update(self.actors_target[i], self.actors[i], self.tau)  # todo soft or hard
