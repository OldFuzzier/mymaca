...
def fighter_rule(tmp_course, tmp_pos, tmp_l_missile, tmp_s_missile, tmp_r_visible_pos, tmp_r_visible_dis, tmp_j_visible_dir,
                 tmp_j_visible_fp, tmp_g_striking_pos, step_cnt):
    """
    飞机打击规则
    ...
    # logger.info('tmp_pos:{}'.format(tmp_pos))
    # logger.info('tmp_r_visible_pos:{}'.format(tmp_r_visible_pos))
    # logger.info('tmp_r_visible_dis:{}'.format(tmp_r_visible_dis))
    ...
    if len(farr[0]) > 0:
        ...
        elif tmp_l_missile[0] > 0:  # todo del distance
            true_action[3] = action_id
        elif tmp_l_missile[0] == 0 and tmp_s_missile[0] == 0:
            true_action[0] = 180 + getarc360(int(math.atan2(fight_pos[1] - tmp_pos[1], fight_pos[0] - tmp_pos[0]) * 180 / math.pi))
            true_action[2] = 11
