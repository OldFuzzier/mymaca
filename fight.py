...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent1", type=str, default="maddispersedpg", help='agent 1 name, only name, not path')  # todo
    ...
    parser.add_argument("--max_step", type=int, default=1000, help='max step in a round')  # todo
    parser.add_argument("--random_pos", default=True, action="store_true",
                        help='if the initial positions are random or fix')
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    # todo 更改敌方动作
                    # for i in range(len(side2_fighter_action)):
                    #     if args.agent2 == 'fix_rule':
                    #         # 规则
                    #         # side2_fighter_action[i]['r_fre_point'] = i+1
                    #         side2_fighter_action[i]['r_fre_point'] = random.randint(0, 11)
                    #     else:
                    #         # model
                    #         # side2_fighter_action[1] = i+1
                    #         side2_fighter_action[i][1] = random.randint(0, 11)
                    # logger.debug('side2 fighter2 actions: {}'.format(side2_fighter_action[1]))  # todo

                    logger.info('side2 actions:')
                    for i, act in enumerate(side2_fighter_action):
                        logger.info('fight_{}:{}'.format(i+1, act))
                except:
                    env.set_surrender(1)
                    # reward
                    ...
                    # wait = 0
                    # print('='*50)
                    # for i in range(side1_fighter_num):
                    #     if not side1_fighter_obs_raw_list[i]['alive']:
                    #         continue
                    #     if side1_fighter_obs_raw_list[i]['r_visible_list']:
                    #         wait = 1
                    #         print(i + 1, 'r_r_visible', side1_fighter_obs_raw_list[i]['r_visible_list'])
                    #     if side1_fighter_obs_raw_list[i]['j_recv_list']:
                    #         wait = 1
                    #         print(i + 1, 'r_j_rev_list', side1_fighter_obs_raw_list[i]['j_recv_list'])
                    #     # if side1_joint_obs_raw_dict['passive_detection_enemy_list']:
                    #     #     print(i + 1, 'r_passive_detection_enemy_list', side1_joint_obs_raw_dict['passive_detection_enemy_list'])
                    # for i in range(side2_fighter_num):
                    #     if not side2_fighter_obs_raw_list[i]['alive']:
                    #         continue
                    #     if side2_fighter_obs_raw_list[i]['r_visible_list']:
                    #         wait = 1
                    #         print(i + 1, 'b_r_visible', side2_fighter_obs_raw_list[i]['r_visible_list'])
                    #     if side2_fighter_obs_raw_list[i]['j_recv_list']:
                    #         wait = 1
                    #         print(i + 1, 'b_j_rev_list', side2_fighter_obs_raw_list[i]['j_recv_list'])
                    #     # if side2_joint_obs_raw_dict['passive_detection_enemy_list']:
                    #     #     print(i + 1, 'b_passive_detection_enemy_list', side2_joint_obs_raw_dict['passive_detection_enemy_list'])
                    # print('='*50)
                    # if wait: logger.wait()
            ...
            if env.get_done():
                ...
                if o_game_reward > e_game_reward:
                    ...
                    print('DRAW!!!')
                    draw_times += 1
                print('Side 1 total step reward: %d, Side 2 total step reward: %d' % (
                ...
                print(
                    'Side 1 alive detector: %d, Side 1 alive fighter: %d, Side 2 alive detector: %d, Side 2 alive fighter: %d' % (
                        side1_detector_alive_num, side1_fighter_alive_num, side2_detector_alive_num,
                        side2_fighter_alive_num))
                time.sleep(2)
                break

    print('FIGHT RESULT:')
    print('Total rounds: %d. Side1 win: %d. Side2 win: %d. Draw: %d' % (
        round_cnt, side1_win_times, side2_win_times, draw_times))
    print('Side 1 win rate: %.1f%%, Side 2 win rate: %.1f%%.' % (
        side1_win_times / round_cnt * 100, side2_win_times / round_cnt * 100))
