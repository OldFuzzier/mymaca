...
class ObsConstruct:
    ...
    def obs_construct(self, obs_raw_dict):
        ...
        detector_r_visible_pos, fighter_r_visible_pos = self.__get_r_visible(detector_data_obs_list,
                                                                             fighter_data_obs_list)
        ...
        detector_j_visible_dir, fighter_j_visible_dir = self.__get_j_visible_dir(detector_data_obs_list,
                                                                                 fighter_data_obs_list)
        fighter_striking_id = self.__get_striking_id(fighter_data_obs_list)
        joint_j_visible_pos = self.__get_joint_visible_pos(joint_data_obs_dict)
        ...
        for x in range(self.detector_num):
            ...
            g_visible_pos = joint_j_visible_pos[0, :]
            ...
            detector_obs_list.append({'course': copy.deepcopy(course), 'pos': copy.deepcopy(pos),
                                      ...
                                      'g_visible_pos': copy.deepcopy(g_visible_pos)})
        # 战机
        for x in range(self.fighter_num):
            ...
            striking_id = fighter_striking_id[x, :]
            g_visible_pos = joint_j_visible_pos[0, :]
    ...
    def __get_r_visible(self, detector_data_obs_list, fighter_data_obs_list):
        ...
        return detector_r_visible_pos, fighter_r_visible_pos
    ...
    def __get_striking_id(self, fighter_data_obs_list):
        fighter_striking_id = np.full((self.fighter_num, 10, 1), 0,
                                      dtype=np.int32)  # todo : fighter_num + detector_num(12)
        for x in range(self.fighter_num):
            if fighter_data_obs_list[x]['alive']:
                for y in range(len(fighter_data_obs_list[x]['striking_list'])):
                    if fighter_data_obs_list[x]['striking_list']:
                        ...
                        fighter_striking_id[x, j_id - 1, 0] = 1

        return fighter_striking_id
